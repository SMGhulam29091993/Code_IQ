...
export const RepoCard: FC<RepoCardProps> = ({ repo, onToggle, isToggling, overFreeLimit }) => {
  ...
  // The toggle below is a real <button>, so the row itself can't also be an interactive
  // element (role="button" wrapping a <button> is an axe "nested-interactive" violation —
  // screen readers can't sensibly represent a control inside a control). Instead, only the
  // navigable cells are wrapped in a button, styled with `contents` so it drops out of the CSS
  // Grid box model entirely and its children lay out as direct grid items of the row below —
  // the toggle stays a sibling, not a descendant.
  return (
    <div
      style={{ gridTemplateColumns: REPO_GRID_COLS }}
      className="grid items-center gap-4 border-b border-white/5 px-5 py-3.5 last:border-b-0 hover:bg-surface2"
    >
      <button
        type="button"
        ...
        <div className="flex min-w-0 items-center gap-2.5">
          <span
            className={cn(
              ...
              repo.isActive ? "bg-accent" : "bg-text3"
            )}
          />
