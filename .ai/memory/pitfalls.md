...
## 016 — BullMQ rejects a custom `jobId` containing `:`, silently breaking the *entire* chunk-fanout pipeline
**Symptom:** Every real, webhook-triggered review failed with `Error: Custom Id cannot contain :` thrown from `FlowProducer.addChildren` — but all 342+ unit/integration tests for `review-coordinator.job.ts` and `review.service.ts`'s `retryReview` passed clean, because they all mock `FlowProducer`/`reviewFlowProducer` entirely and never exercise BullMQ's own job-ID validation. Found 2026-09-06 triggering a real coordinator run against real Redis/BullMQ for the first time since Step 8 shipped (2026-08-30) — the containers had been running stale, pre-Step-8 code the whole time until that same session's rebuild, so this had never actually run for real before.
**Root cause:** `review-coordinator.job.ts` and `review.service.ts` both built BullMQ child `jobId`s as `` `${review.id}:${row.id}` `` / `` `${reviewId}:${chunk.id}:retry${chunk.attempts}` ``. `package.json` pins `"bullmq": "^5.21.0"` (a caret range); the version actually installed, `5.80.8`, added validation rejecting any custom `jobId` containing `:` — a breaking behavior change within the same major version, invisible to a caret-range dependency and to a test suite that mocks the library it should have caught this in.
**Fix:** Both job-ID templates changed to use `-` instead of `:` as the separator (cuids never contain either character, so uniqueness is unaffected). Nothing parses these IDs back apart anywhere in the codebase — confirmed via a full-repo search — so this was a pure separator swap, no other call sites affected. Three test files had hardcoded the old `:`-containing string as an exact-match assertion (`review-coordinator.job.test.ts`, `review.service.test.ts`, `review.routes.test.ts`) and needed updating too — a reminder that even an exact-string assertion on a mocked call doesn't protect against the real library rejecting that exact string. **Takeaway: a caret-range pin on a library whose behavior your tests only ever mock can drift underneath you with zero red tests** — this specific class of bug is only catchable by periodically exercising the real dependency, not by unit tests alone.

## 017 — Switching git branches doesn't revert the gitignored generated Prisma client
**Symptom:** After a `git checkout` to a different branch (found 2026-09-12 when the working directory ended up on `fix/review-coordinator-idempotency` mid-session, apparently from an external `git checkout` outside this session's control — IDE or the user's own terminal), `pnpm --filter @codeiq/api typecheck` failed with `Type 'bigint | null' is not assignable to type 'number | null'` in `review.service.ts` — a file with no uncommitted changes, on a branch whose own `schema.prisma` correctly declares `githubReviewId` as `Int?`, not `BigInt?`.
**Root cause:** `packages/db/generated/client` (Prisma's generated output, gitignored per the root `.gitignore`) is a build artifact, not source — `git checkout` only touches tracked files. A `prisma generate` run on one branch (here, `fix/github-review-id-overflow`, which does have the `BigInt` migration) leaves its generated types sitting on disk even after switching to a branch whose `schema.prisma` disagrees. TypeScript then checks real source code against a generated client that describes a different branch's schema.
**Fix:** No code fix — this is a workflow gotcha, not a bug. Run `pnpm --filter @codeiq/db exec prisma generate` (or `pnpm --filter @codeiq/db build`, which does it as a first step) immediately after any `git checkout`/branch switch that could have touched `schema.prisma` on either side, before trusting `tsc`'s output. Worth a specific gut-check whenever a typecheck error names a field/type that doesn't match what `schema.prisma` on disk actually says — that mismatch is the signature of this exact issue, not a real code defect.
