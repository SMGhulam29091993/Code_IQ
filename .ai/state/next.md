# Next Tasks
> Queued — not yet started.

1. Auth screens — register (2-step w/ OTP), login, AuthProvider rehydration (frontend Step 2)
2. Install flow (frontend Step 3) — can now call the completed `/github/*` and `/repos/*` APIs
3. Review dashboard screens (frontend) — can now call the completed `/reviews/*` APIs
4. Billing screens (frontend) — can now call the completed `/billing/*` APIs
5. Deploy (backend Step 7) — AWS EC2/RDS/ElastiCache, production webhook URL, Secrets Manager
   (Dockerfiles, docker-compose.yml, and `GET /health` are done — see `plans/backend.md` Step 7)
